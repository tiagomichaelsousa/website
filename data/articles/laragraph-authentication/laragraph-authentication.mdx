---
title: '#1 - Laragraph Authentication - Laravel and GraphQL'
date: '2022-03-30'
image: thumbnail.jpeg
slug: 'articles/laragraph-authentication'
authors:
  - tsousa
description: |
  This article is the first article of a serie that I'm writing about two main tools that I absolutely love! Laravel and GraphQL.
  During this serie we will create a tipical blog application while exploring all the features available from Lighthouse, the tool that will add GraphQL support to our Laravel application.
...
understand how could I create an applciation with a GraphQL API instead of a a tipical REST api.
...
and retreieving onyl what really matters.
...
Sail will be installed as a `dev` dependency, so you will always have access to the CLI from the project directory but it's kind of annoying being always
writing `./vendor/bin/sail` in your terminal each time that you want to use the Sail CLI to interact with your application.
For that, you can easily create an alias within your bash profile like bellow:
...
For this serie of articles, we will use Sanctum for our authentication system to authenticate users with our GraphQL API.
...
To do that, go to you `config/lightouse.php` and update your guard like bellow:
...
Every "endpoint" is should be present in our GraphQL schema, so let's start by creating our login mutation.
...
For this serie, we will use Pest to write our tests and the first one that we will write is for our login functionality.
...
Since this serie will be a very simple intro to Laravel and GraphQL, we will not develop the entire registration flow.
